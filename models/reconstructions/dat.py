...
class Spatial_Attention(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        print(f'Spatial_Attention{x.size()}')
...
class Adaptive_Channel_Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        print(f'Adaptive_Channel_Attention{x.size()}')
...
def get_cosine_dissimilarity(q, s):
    bsize, ch_sz, _ = q.size()[:3]

    # Flatten spatial dimensions
    q_flat = q.view(bsize, ch_sz, -1)
    s_flat = s.view(bsize, ch_sz, -1)

    # Compute norms
    q_norm = q_flat.norm(p=2, dim=1, keepdim=True)
    s_norm = s_flat.norm(p=2, dim=1, keepdim=True)

    # Normalize the vectors
    q_normalized = q_flat / q_norm
    s_normalized = s_flat / s_norm

    # Compute cosine similarity
    cosine_similarity = (q_normalized * s_normalized).sum(dim=1)
    # cosine_similarity = torch.bmm(q_normalized.transpose(1, 2), s_normalized)

    # Convert to cosine dissimilarity
    cosine_dissimilarity = 1 - cosine_similarity

    # Taking maximum dissimilarity across the channel dimension and reshape
    dissimilarity = cosine_dissimilarity.view(bsize, 1, q.size(2), q.size(3))
    # max_dissimilarity = cosine_dissimilarity.max(2)[0].view(bsize, 1, q.size(2), q.size(3))

    return dissimilarity


def compute_anomaly_score(support, query):
    anomaly_map = torch.pow(support - query, 2).mean(1, keepdim=True)
...
if __name__ == '__main__':
    ...
    image_path1 = 'D:/data/industrial5/data/bottle/good/test_good_000.png'  # D:\data\industrial5\data\bottle\good
    image_path2 = 'D:/data/industrial5/data/bottle/good/test_good_001.png'
    image_path3 = 'D:/data/industrial5/data/bottle/good/test_good_002.png'
    bad_image_path = 'D:/data/industrial5/data/bottle/bad/test_broken_large_003.png'
    bad_mask = 'D:/data/industrial5/data/bottle/bad/test_broken_large_003.bmp'
    output_path = 'anomaly_score.png'

    image1 = Image.open(image_path1).convert("RGB")
    image2 = Image.open(image_path2).convert("RGB")
    image3 = Image.open(image_path3).convert("RGB")
    image_bad = Image.open(bad_image_path).convert("RGB")
    mask = Image.open(bad_mask).convert("L")
    # mask = np.array(mask)

    T = transforms.Compose([transforms.Resize([128, 128]), transforms.ToTensor()])
    mask = T(mask)
    input_normal = torch.stack([T(image2), T(image3)])
    input_bad = torch.stack([T(image3), T(image_bad)])

    x = torch.randn((2, 256, 14, 14), dtype=torch.float32).cuda()
    y = model(x)
    print(y.shape,'@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')


    # print('input shape:::::::::::',input.shape)
    # x = torch.randn((3, 3, height, width)).cuda()
    # x1 = torch.randn((1,3,512, 512))
    norm_features = model(input_normal)
    norm_features = norm_features.mean(dim=0)  # .unsqueeze(dim=0)
    print('norm_features shape:::::::::::', norm_features.shape)

    anomalous_features = model(input_bad)
    anomalous_features = anomalous_features.mean(dim=0)  # .unsqueeze(dim=0)
    print('anomalous_features shape:::::::::::', anomalous_features.shape)

    anomaly_score = compute_anomaly_score(norm_features, anomalous_features)
    # anomaly_score = get_cosine_dissimilarity(anomalous_features, norm_features)
    print('anomaly_score shape:::::::::::', anomaly_score.shape)

    # dissimilarity = get_dissimilarity(anomalous_features, norm_features, mask)
    # print(dissimilarity.shape)

    # print(x.shape)
    transform_to_pil = transforms.ToPILImage()
    result_image = transform_to_pil(anomaly_score.squeeze().cpu())

    # Save or display the result
    result_image.save(output_path)

    # for layer in residual_layers:
    #     y = layer(x1, x1.shape[2:])
    #     print(y.shape)

    # print(x.shape)
    # print(residual_layers)
