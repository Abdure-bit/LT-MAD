...
class Converter(nn.Module):
    ...
    def __init__(self, dim_in=256, dim_temp=128, img_size=224, mids=4):
        ...
        # Update f1 and f2 layers for 256 channels
        self.norm_layer_f1 = nn.LayerNorm(dim_in)
        self.conv_f1_Q = nn.Conv2d(self.dim_in, self.dim_temp, kernel_size=1)
        self.conv_f1_K = nn.Conv2d(self.dim_in, self.dim_temp, kernel_size=1)
        self.ap_f1 = nn.AdaptiveAvgPool2d(output_size=(mids + 2, mids + 2))
    ...
    def forward(self, token_pair):
        ...
        for index in range(len(token_pair) // 2):
            ...
            f1_ = self.norm_layer_f1(token_pair[index * 2][:, :, :])  # [8,194,256]
            f2_ = self.norm_layer_f2(token_pair[index * 2 + 1][:, :, :])  # [8,194,256]
...
if __name__ == "__main__":
    model = Model(ckpt=None)  # No checkpoint for testing
    model.eval()  # Set model to evaluation mode

    # Create random input tensor [batch_size, channels, height, width]
    random_input = torch.randn(1, 3, 384, 384)
