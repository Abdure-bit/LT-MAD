...
from models.reconstructions.dat import Adaptive_Channel_Attention,Adaptive_Spatial_Attention,SGFN,DAT
...
class UniAD(nn.Module):
    ...
    def forward(self, input):
        ...
        print(input_dat.shape,'input_dat ==========================>>>>>>>>>>>>>>>>>.')

        dat_out = self.dat(input_dat)
        print(dat_out.shape, 'DAT out ==========================>>>>>>>>>>>>>>>>>.')
...
class Transformer(nn.Module):
    ...
    def forward(self, src, pos_embed):
        ...
        print(output_encoder.shape,'output enocder shape<<<<<<<<<<<<<<<<<<<<')
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        #         hidden_dim, num_heads=nhead, reso=reso, split_size=split_size, shift_size=shift_size,
        #         qkv_bias=qkv_bias,
        #         qk_scale=qk_scale,
        ...
        #         hidden_dim, num_heads=nhead, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
        #         proj_drop=drop
        #     )
        # # self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        # # ffn_hidden_dim = int(hidden_dim * expansion_factor)
        # # self.ffn = SGFN(in_features=hidden_dim, hidden_features=ffn_hidden_dim, out_features=hidden_dim,
        # #                 act_layer=act_layer)
